...
def _ensure_runtime_imports() -> None:
    ...
    global SignalProcessing
    global SkinExtractionConvexHull
    global SkinExtractionFaceParsing
    global sig_windowing
    global sliding_straded_win_idx
    global RGB_sig_to_BVP
    global BPfilter
    global apply_filter
    global BVP_to_BPM
    global BPM_median
    ...
    # Import heavy pyVHR/MediaPipe stack only when the API is actually invoked.
    ...
    from pyVHR.extraction.utils import sig_windowing, sliding_straded_win_idx
...
def _cupy_cuda_available():
    if os.environ.get("SACCARD_DISABLE_GPU", "").strip().lower() in {"1", "true", "yes"}:
        return False
    ...
    try:
        ...
        return bool(runtime.getDeviceCount() > 0)
    except Exception:
        return False
...
def saccard(
    ...
    stream_duration=30,
    max_frames=None,
    holistic_downsample_scale=1.0,
    holistic_landmark_refresh_stride=3,
    verb=False,
):
    """
    ...
    holistic_downsample_scale : float
        FaceMesh inference scale factor in (0, 1]. Lower values speed up CPU
        landmarking; 1.0 preserves original behavior.
    holistic_landmark_refresh_stride : int
        Re-run FaceMesh every N frames and reuse the most recent landmarks
        between refreshes (default 3).
    verb : bool
    ...
    if max_frames is None:
        ...
        sig_proc.set_total_frames(int(max_frames))
    sig_proc.set_holistic_speedup(
        downsample_scale=holistic_downsample_scale,
        landmark_refresh_stride=holistic_landmark_refresh_stride,
    )
    ...
    try:
        ...
        raise RuntimeError(
            "Holistic RGB extraction failed in SignalProcessing.extract_holistic. "
            f"video={video_source!r}, fps={fps!r}, max_frames={max_frames!r}, "
            f"holistic_downsample_scale={holistic_downsample_scale!r}, "
            f"holistic_landmark_refresh_stride={holistic_landmark_refresh_stride!r}"
        ) from exc
    if sig is None or len(sig) == 0:
        raise RuntimeError(
            "Holistic RGB extraction returned no samples. "
            f"video={video_source!r}, fps={fps!r}, max_frames={max_frames!r}"
        )
    ...
    if len(windowed_sig) == 0:
        duration_s = len(sig) / max(float(fps), 1e-6)
        raise RuntimeError(
            "No analysis windows were generated. "
            f"frames={len(sig)}, fps={fps:.4f}, winsize_s={winsize}, duration_s={duration_s:.4f}. "
            "Use a smaller winsize or a longer video."
        )
    ...
    if run_ssr and raw_frames is not None and len(raw_frames) > 0:
        ...
        if len(raw_windowed_sig) == 0:
            raise RuntimeError(
                "SSR requested but no raw-frame windows were generated. "
                f"frames={len(raw_frames)}, fps={fps:.4f}, winsize_s={winsize}."
            )
